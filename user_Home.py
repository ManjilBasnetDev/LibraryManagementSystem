...
def main():
    ...
    # Home Page
    def home_page():
        label = CTkLabel(main_frame, text="Welcome to the E-Library", font=("Arial", 24, "bold"), text_color='black')
        label.pack(pady=20)
        
        desc = CTkLabel(main_frame, text="Your digital library for books and resources.", font=("Arial", 14), text_color='black')
        desc.pack(pady=10)
    ...
    def books_available():
        label = CTkLabel(main_frame, text="Books Available", font=("Arial", 24, "bold"), text_color='black')
        label.pack(pady=10)
    ...
    def buy_books():
        def generate_bill():
            ...
            bill_window = CTkToplevel(main_frame)
            bill_window.title("Book Purchase")
            bill_window.geometry("350x400")

            # Display book details
            CTkLabel(bill_window, text="Book Details", font=("Arial", 20, "bold")).pack(pady=10)
            details = f"Title: {book_info[0]}\nAuthor: {book_info[1]}\nGenre: {book_info[2]}\nPrice per book: $20"
            CTkLabel(bill_window, text=details, font=("Arial", 14)).pack(pady=5)

            # Quantity selection
            CTkLabel(bill_window, text="Select Quantity:", font=("Arial", 14)).pack(pady=5)
            ...
            # Total price label
            total_price_label = CTkLabel(bill_window, text="Total Price: $20", font=("Arial", 16, "bold"))
            ...
            # Buttons
            CTkButton(bill_window, text="Print Bill", command=print_bill).pack(pady=5)
            CTkButton(bill_window, text="Confirm Purchase", fg_color="green", command=confirm_purchase).pack(pady=5)

        # UI for book selection
        label = CTkLabel(main_frame, text="Buy Books", font=("Arial", 24, "bold"), text_color='black')
        label.pack(pady=10)
        ...
        CTkButton(main_frame, text="Generate Bill", command=generate_bill).pack(pady=10)

    # Contact Us Page
    def contact_us():
        label = CTkLabel(main_frame, text="Aryan Shrestha", font=("Arial", 24, "bold"), text_color='black')
        label.pack(pady=20)
        
        CTkLabel(main_frame, text="Email: support@elibrary.com\nPhone: +123456789", font=("Arial", 14), text_color='black').pack(pady=10)

    # About Us Page
    def about_us():
        label = CTkLabel(main_frame, text="About Us", font=("Arial", 24, "bold"), text_color='black')
        label.pack(pady=20)
        
        CTkLabel(main_frame, text="Our team\n1.Aryan Shrestha\n2.Manjil Basnet\n3.Abhishek Khatiwada\n4.Arbaz Rain", font=("Arial", 14), text_color='black').pack(pady=10)

    # Sidebar Navigation
    sidebar = CTkFrame(root, width=200, corner_radius=10, fg_color='#dfd8ee')
    ...
    for text, command in buttons:
        btn = CTkButton(sidebar, text=text, command=lambda cmd=command: show_frame(cmd), font=("Arial", 14), corner_radius=10)
        btn.pack(pady=20, padx=15)

    # Main Content Area
    main_frame = CTkFrame(root, fg_color="#dfd8ee")
